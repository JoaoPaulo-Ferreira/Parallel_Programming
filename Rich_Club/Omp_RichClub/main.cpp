...
int main(int argc, char const **argv){
	...
	string input;
	int vertex, edge, node_A, node_B,  k_max=0, nEdges=0, rc_size=0;
	double rc_coef;
	struct timeval t1, t2,t3,t4;
	infile >> vertex >> edge;
	cout << "v = " << vertex << " e = " << edge << endl;
	...
	*/
	...
	cout << "Entrando na seção paralela " << endl;
// Abrindo uma seção paralela para encontrar o clube dos ricos
	// O método size() é thread-safe contanto que não haja race condition no atributo e para garantir isso precisamos de um critical

	gettimeofday(&t3, 0);
#pragma omp parallel for default(none) shared(graph, vertex, k, rich_club)
		for(int j = 0; j < vertex; j++){ 
			if( (int) graph[j].size() > k)
				#pragma omp critical // O método push_back() não é thread-safe então precisamos garantir que não haja race condition
				rich_club.push_back(j);
		}
	gettimeofday(&t4, 0);
	printf("It took %f milliseconds in push_back.\n",(t4.tv_sec - t3.tv_sec) + (t4.tv_usec - t3.tv_usec) / 1e6);

		
		rc_size = rich_club.size(); // tamanho do rich club
		if(rc_size > 1){

#pragma omp parallel for default(none) shared(rich_club, graph) reduction(+:nEdges)
